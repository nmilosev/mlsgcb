    ...
    def addTopk(self, topi, topv, decoder_hidden, beam_size, voc):
        ...
        for i in range(beam_size):
            if topi[0][i] == EOS_token:
                terminates.append(([voc.index2word[idx].item() if idx in voc.index2word else '<UNK>' for idx in self.sentence_idxes] + ['<EOS>'],
    ...
    def toWordScore(self, voc):
        ...
        for i in range(len(self.sentence_idxes)):
            if self.sentence_idxes[i] == EOS_token:
                words.append('<EOS>')
            elif self.sentence_idxes[i] not in voc.index2word:
                words.append('<UNK>')
            else:
...
def evaluate(encoder, decoder, voc, sentence, beam_size, max_length=MAX_LENGTH):
    ...
    indexes_batch = [[voc.word2index[word] if word in voc.word2index else 3 for word in sentence.split(' ')]]
...
def evaluateInputWeb(encoder, decoder, voc, beam_size):
	app = Flask(__name__)
	import sys
	...
	def ask():
		message = request.form['message'].lower().replace('?', ' ?')
		print("got message:", message)
		output_words, _ = evaluate(encoder, decoder, voc, message, beam_size)
		output_sentence = ' '.join(output_words)
		return json.dumps({'q': message, 'a': output_sentence})

	app.run(host='0.0.0.0', port=39249)
...
def runTest(n_layers, hidden_size, reverse, modelFile, beam_size, inp, corpus):
    ...
    if inp:
        ...
        evaluateInputWeb(encoder, decoder, voc, beam_size)
    else:
